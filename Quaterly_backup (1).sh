...
if [ $CURRENT_MONTH -eq 1 ]; then
    ...
    echo "No backup scheduled for the current month quarter has not yet started or ended..."
    exit 1
fi
...
# Cleanup old backups, keep only the last 15 months' files (approximately 16*31 = 496 days)
find /opt/MySQLDUMPS/ -name "MySQL_Reportdata_*.sql" -type f -mtime +496 -exec rm -f {} \; -exec echo "Removed old backup file: {}" >> "$BACKUP_LIST_FILE" \;
